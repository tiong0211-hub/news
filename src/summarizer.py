...
도움이 되는 뉴스를 우선하세요.
5. 최종 선정 결과는 주제가 겹치지 않도록 다양하게 구성하세요. 같은 기업/업종/사건을 다루는 기사가 \
여러 개 있어도 최종적으로는 그 중 가장 중요한 1개만 선택하고, 나머지 자리는 서로 다른 주제 \
(예: 금리, 환율, 특정 기업 실적, 부동산, 원자재, 정책 등)의 기사로 채우세요. 상위 기사 2~3개가 \
사실상 같은 뉴스를 다루는 상황은 피하세요.
6. 최종적으로 중요도 순으로 최대 {top_n}개를 선정하세요. 후보가 부족하면 있는 만큼만 반환하세요.
