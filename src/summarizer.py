...
6. 최종적으로 중요도 순으로 정확히 {top_n}개를 선정하는 것을 목표로 하세요. 후보 중 유효한 \
(중복이 아니고 투자와 무관하지 않은) 기사가 {top_n}개 이상이면 반드시 {top_n}개를 채우세요. \
개수를 채우기 위해 우선순위가 낮은 기사를 포함해야 하더라도 {top_n}개를 채우는 것이 우선입니다. \
같은 대분류 주제(예: 금리)의 기사가 여러 개 남아야 개수를 채울 수 있다면 그렇게 하세요 — \
주제 다양성보다 개수를 맞추는 것이 우선순위가 높습니다. 정말로 유효한 후보 자체가 {top_n}개 \
미만일 때만 있는 만큼 반환하세요.
